...
final class SQLiteMedicalAttachmentRepository extends AbstractRepository implements MedicalAttachmentRepositoryInterface
{
    ...
    public function save(MedicalAttachment $a): void
    {
        $stmt = $this->pdo->prepare(
            "INSERT INTO medical_attachments (id, tenant_id, record_id, filename, mime_type, size_bytes, storage_key, uploaded_by, created_at)
             VALUES (:id, :tenant_id, :record_id, :filename, :mime_type, :size_bytes, :storage_key, :uploaded_by, datetime('now'))"
        );
        $stmt->execute([
            'id'          => $a->getId(),
