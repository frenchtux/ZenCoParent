...
final class SQLiteMessageRepository extends AbstractRepository implements MessageRepositoryInterface
{
    ...
    public function findByThreadId(
        ...
        return array_map(static fn(array $row): Message => Message::fromArray($row), $stmt->fetchAll());
    }
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE messages SET read_at = :read_at WHERE id = :id');
        $stmt->execute(['read_at' => $readAt->format('Y-m-d H:i:s'), 'id' => $messageId]);
    }
    ...
    public function countUnread(string $threadId, string $excludeSenderId): int
    {
        ...
        $stmt->execute(['thread_id' => $threadId, 'sender_id' => $excludeSenderId]);
